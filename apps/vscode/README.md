...
- **A live-provider editor transcript remains manual** — the browser snapshot proves the built webview renders fixture assistant content, interaction composers, and tool rows at 259px without root-level horizontal scrolling or uncontained overflow; the Extension Host lane proves activation and native integrations against a fixture server. Neither executes a real provider turn inside the editor.
