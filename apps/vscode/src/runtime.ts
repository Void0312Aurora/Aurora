...
export class ServerRuntime {
  ...
  private async performStart(): Promise<URL> {
    ...
    const configuredLaunch = { ...launch, args: [...launch.args, '--app-config', options.appConfigPath] }
    options.log(`launching dsh web (${launch.source}): ${launch.command} ${configuredLaunch.args.join(' ')}`)
    const child = spawnWebLaunch(configuredLaunch, {
