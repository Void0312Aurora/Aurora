...
VS Code Webview 载体使用受限的 `postMessage` bridge：运行时解析器拒绝格式错误的请求与响应，重复 id 会让所属请求失败，流取消会中止宿主 fetch，每个请求监听器都会在完成、错误、取消或销毁时移除。连接握手会先校验 `host.describe.protocolVersion`，再打开任一事件流，因此独立发布的 Webview 不会消费不兼容宿主发送的帧。
