...
describe('parseLspArgs', () => {
  ...
  it('rejects a blank file_path', () => {
    expect(() => { return parseLspArgs({ operation: 'hover', file_path: '   ', line: 1, character: 1 }) })
      .toThrow(/file_path/)
  })
  ...
  it('rejects non-positive or non-integer coordinates', () => {
    ...
    expect(() => { return parseLspArgs({ operation: 'hover', file_path: 'a.ts', line: 1, character: 0 }) }).toThrow(/character/)
