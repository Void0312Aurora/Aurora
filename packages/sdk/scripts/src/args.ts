...
export function parseDshSdkArgs(argv: readonly string[]): DshSdkArgs {
  ...
  const launcherArgv = separator !== -1 ? argv.slice(0, separator) : argv
  ...
  if (parsed.command === 'config' && passthrough.length > 0) throw new Error('dsh-sdk config does not accept forwarded arguments')
