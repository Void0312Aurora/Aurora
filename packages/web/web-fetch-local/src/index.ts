...
function assertTimeoutMs(value: number): void {
  ...
  if (value > MAX_NODE_TIMER_DELAY_MS) throw new Error(`web-fetch-local: timeoutMs must be no greater than ${MAX_NODE_TIMER_DELAY_MS}`)
}

/** The redirect hop cap must be a non-negative integer (0 follows no redirects). */
function assertNonNegativeInteger(name: string, value: number): void {
  if (!Number.isInteger(value) || value < 0) throw new Error(`web-fetch-local: ${name} must be a non-negative integer`)
}
