...
function brandColorAt(t: number): readonly [number, number, number] {
  ...
  const from: readonly [number, number, number] = BRAND_GRADIENT[index] as readonly [number, number, number]
...
export function renderPalette(
  ...
  const purpose = (role: RoleSpec): string => { return `  ${palette.dim(`    ${role.purpose}`)}` }
  const rows = [
