...
Editor movements feed the active session bounded context that a prompt can refer to, without waking the model and without flooding it. The sampler (`ide-context.spec.ts`), feed (`context-feed.spec.ts`), active-session tracker (`active-session.spec.ts`), and bridge barrier (`bridge.spec.ts`) are covered keyless over injected editor state, client, scheduler, and deferred relay. The assembled `@vscode/test-electron` lane opens a file before activating the built extension, drives a real prompt through the built webview and production `dsh web` composition with a replayed LLM, and reads the production JSONL to require IDE context before the user prompt and first request header. The known approximation applies only to background editor nudges when multiple sessions are attached; the first bridged prompt uses its explicit session. Bounds (`maxTextChars`, `maxDiagnostics`, the cursor window) are fixed constants in `extension.ts` for v1; they become settings when the extension grows a settings surface.
