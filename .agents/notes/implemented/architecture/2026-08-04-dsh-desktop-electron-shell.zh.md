...
**托盘常驻，而不是窗口生命周期。** 关闭窗口只隐藏它，服务器继续运行；托盘菜单重新打开窗口或退出（同时杀掉服务器）。二次启动聚焦已有窗口。打包版不需要 Node、`dsh` 或检出目录：嵌入闭包在 Electron-as-Node（`ELECTRON_RUN_AS_NODE=1`）下运行，闭包内的原生插件（node-pty、koffi）是 N-API，无需针对 Electron 重新编译（`npmRebuild: false`）。

**打包复用仓库已验证的 deploy 模式。** `apps/desktop/closure` 是纯依赖 deploy root（`@deepseek-ai/dsh` + `@deepseek-ai/dsh-frontend`），由 `pnpm deploy` 物化到 `apps/desktop/deploy`，与 `python/sdk-runtime` 完全同型；`apps/web` 增加了 `files: ["dist"]` 字段，让构建好的 GUI 进入闭包。electron-builder 只打包 `lib/`、`build/`（图标）和 `deploy/`；本包自身零运行时依赖，因此 electron-builder 对 node_modules 的处理永远不会碰到闭包。`dist` 先跑完整仓库构建（`build:lib` + `build:web`），因为闭包复制的是构建产物而非源码。
