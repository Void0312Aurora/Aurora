...
**The vsix is per-platform.** The closure carries N-API native addons (node-pty, koffi), so `vsce package --target <target>` produces one vsix per platform; a CI matrix packs the set. Native addons are ABI-compatible with Electron-as-Node, so no rebuild.
