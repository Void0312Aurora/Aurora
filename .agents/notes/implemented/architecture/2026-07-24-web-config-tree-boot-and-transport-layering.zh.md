...
- 嵌入器可以静态打包图中插件，而无需绕过 Loader 生命周期或取代启动 manifest 的花名册权威；有效静态行不会获取或执行组合包。
