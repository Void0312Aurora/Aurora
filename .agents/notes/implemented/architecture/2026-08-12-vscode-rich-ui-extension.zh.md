...
启动器抽取使 `apps/desktop` 成为 `@deepseek-ai/dsh-web-launcher` 的消费者（其内联 `launcher.ts` 与单测套件整体迁入新包）。`PostMessageApiClient`、`AppWebEntry` 的静态插件 seam 与主题适配器都有无密钥覆盖：传输的一元/SSE/abort/清理行为在 `packages/client/connection/tests/webview-bridge.spec.ts`，平台选择在 `client-apply.spec.ts`。扩展宿主自身逻辑经注入的 spawn/fetch/kill seam 无密钥测试（`apps/vscode/tests/runtime.spec.ts` 含 dispose 取消进行中的就绪轮询、`bridge.spec.ts` 含 SSRF 逃逸拒绝、`panel.spec.ts`），`roster.spec.ts` 在静态 roster 与已交付 web 配置漂移时让构建失败。以上没有一项能看见"下载成功随后抛错"的 bundle，因此 `apps/vscode/tests/webview-boot.e2e.ts` 加入浏览器通道（`vitest.web.config.ts`）：它把真实 `dist/webview` 产物经真实 `panelHtml()` 文档（含已交付的 CSP）提供出去，除非 React 挂载且无未捕获错误否则失败。这一条断言同时钉住上述两项约束；单测无法替代它，因为失败形态正是"bundle 完美加载却什么都不渲染"。在真实编辑器里启动面板仍是手动步骤，直到 `@vscode/test-electron` 通道落地。在打包阶段捆绑服务器闭包之前，扩展依赖启动器解析出一个 `dsh`（DSH_BIN、checkout 或 PATH）；三者皆无的机器会在面板里得到启动错误，而非损坏的 GUI。
