...
- `apps/` holds the externally exported application shapes, assembled from Client / Host mixtures.
    ...
    - The Electron desktop shell (`apps/desktop`) reuses the same web client packages over the `dsh web` HTTP channel at `127.0.0.1:<port>` — no IPC fetch carrier ([dsh-desktop note](2026-08-04-dsh-desktop-electron-shell.md)). The IPC-bridge plan was abandoned because the renderer is a sandboxed window without a preload (an IPC bridge would need a preload plus a context bridge, widening the attack surface), HTTP reuses the existing webserver with zero extra protocol layers, and the browser and desktop shapes share the same readiness and trust boundary. The four-quadrant message model and the rest of this note are unaffected.
...
| Carrier layer | `dsh-host-webserver` | Web-shape HTTP: static serving + `/api/*`→handler forwarding + SSE write-out + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Web (browser access) only; zero workspace dependencies (the registry arrives by structural injection); the desktop shell reuses it over the `dsh web` HTTP channel ([dsh-desktop note](2026-08-04-dsh-desktop-electron-shell.md)) |
...
1. **Pick a fetch impersonation**: browser same-origin HTTP / in-process `host.handler.fetch` injection / your own transport-aspect subclass (e.g. future Electron IPC — **not adopted**: the desktop shell uses the browser `WebApiClient` over HTTP instead, see the [dsh-desktop note](2026-08-04-dsh-desktop-electron-shell.md); see the "Subclass table" below).
...
- **Protocol version**: `host.describe` returns `API_PROTOCOL_VERSION`; independently released clients reject missing or incompatible versions before consuming versioned methods or frames. In-repo clients continue to release with the host. The [wire protocol version and IDE context injection note](2026-08-12-wire-protocol-version-and-ide-context-injection.md) owns the compatibility rule.
- **Reserved-seam discipline**: the map holds only implemented methods; an unknown method fails loud at envelope parse (`bad-request`) — no not-implemented fallback code. The remaining reservation list (implementing = copy the signature into the domain interface + add the map row + add the schema pair) is `task.list` and describe gaining `hostInstanceId`. `session.fork`, `session.injectContext`, `session.rename`, and `llm.models` have graduated into implemented methods.
...
| (future) IPC bridge subclass — **not adopted**: the desktop shell reuses `WebApiClient` over HTTP (see [the dsh-desktop note](2026-08-04-dsh-desktop-electron-shell.md)) | apps/desktop | IPC serialization round trip | swaps only doFetch; contract and base class unchanged |
