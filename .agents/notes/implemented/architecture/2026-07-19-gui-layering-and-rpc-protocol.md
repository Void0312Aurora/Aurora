...
- `apps/` holds the externally exported application shapes, assembled from Client / Host mixtures.
    ...
    - A future Electron shape reuses the same web client packages over an IPC fetch carrier.
        - **Partially superseded by [the dsh-desktop Electron shell note](2026-08-04-dsh-desktop-electron-shell.md)**: the Electron shape landed as `apps/desktop`, reusing the `dsh web` HTTP shell over `127.0.0.1:<port>` — no IPC fetch carrier. The IPC-bridge plan was abandoned because the renderer is a sandboxed window without a preload (an IPC bridge would need a preload plus a context bridge, widening the attack surface), HTTP reuses the existing webserver with zero extra protocol layers, and the browser and desktop shapes share the same readiness and trust boundary. The four-quadrant message model and the rest of this note are unaffected.
...
| Carrier layer | `dsh-host-webserver` | Web-shape HTTP: static serving + `/api/*`→handler forwarding + SSE write-out + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Web (browser access) only; zero workspace dependencies (the registry arrives by structural injection); Electron does not reuse it — **partially superseded**: the desktop shell does reuse it ([dsh-desktop note](2026-08-04-dsh-desktop-electron-shell.md)) |
...
| (future) IPC bridge subclass — **not adopted**: the Electron shape reuses `WebApiClient` over HTTP (see [the dsh-desktop note](2026-08-04-dsh-desktop-electron-shell.md)) | apps/electron | IPC serialization round trip | swaps only doFetch; contract and base class unchanged |
