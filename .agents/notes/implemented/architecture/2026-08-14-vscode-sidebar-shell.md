...
**Navigation is native.** The view contributes into `viewsContainers.secondarySidebar`, finalized in VS Code 1.106 and available to our `^1.125` engine range, so the extension lands in the right-hand sidebar without asking the user to drag it. Title actions own the navigation row: they cost the webview no pixels, the scarcest resource in a narrow column. Their commands post a route message that `webview/route-bridge.ts` turns into a `ctx.layout` call, listening on `window` rather than the api client's bridge port — routing is not wire traffic, and keeping it off that channel leaves the connection package's message union untouched.
...
The sidebar renders the real GUI at a measured 259px with three panes mounted and one active, and the title actions route between them; verified in a real editor through the extension development host. The shell's contract, route semantics, and frame behavior are covered in `apps/vscode/tests/shell.spec.tsx`, and `roster.spec.ts` now asserts the substitution itself — exactly one shell, and it is this one.
