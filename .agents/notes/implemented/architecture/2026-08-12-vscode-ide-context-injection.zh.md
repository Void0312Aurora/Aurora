...
**采样与变化抑制是纯模块；feed 及其 VS Code 接线很薄。** `apps/vscode/src/ide-context.ts` 把普通的 `EditorState`（扩展从 VS Code API 构建）转成有界的 `IdeContextSnapshot`：一段模型面读数加一个作变化检测键的 `signature`（签名就是渲染文本本身，所以经过全部边界后渲染相同的两次采样即是 no-op）。边界在每一层生效——选区/窗口字符数、诊断条数、单条诊断消息上限，以及对完整渲染读数的最终上限，遵循"限制约束完整发出值"的仓库规则。`apps/vscode/src/context-feed.ts` 对编辑器 nudge 防抖、采样、丢弃签名与该会话上次注入相同的快照，并调用 `session.injectContext`——no-wakeup wire 方法，因此读数为会话下一 step 暂存，绝不开启 turn。发送是串行的：同一时刻只有一个注入在途，发送中途到达的 nudge 标记一次尾随重采样而非并发发送，完成顺序不会乱序。这是 tmux-context 插件的 render/suppress 形态搬到宿主侧：在 `agent/step` 上 nudge 变成在编辑器事件上 nudge（活动编辑器、选区、活动文档文本、诊断），"与上次相同读数"的抑制完全一致。把纯核心留在 `vscode` 模块之外，让防抖、抑制、串行化、每会话签名记忆与拒绝重试都能无密钥测试。
...
编辑器移动为活动会话喂入 prompt 可指代的有界上下文，既不唤醒模型也不灌满它。整个原生层（本 feed 加交互消费者）只在扩展宿主用自身版本核验服务器 `protocolVersion` 之后启动——即 wire 笔记定义的独立发布客户端门禁。采样器及其边界（`ide-context.spec.ts`）、防抖/抑制/串行化/重试 feed（`context-feed.spec.ts`）与含变化回调的活动会话跟踪器（`active-session.spec.ts`）经注入的编辑器状态、客户端与调度器无密钥覆盖；`extension.ts` 里的 VS Code 编辑器采样器与事件接线按面板与原生 UI 胶水同理，在 `vscode` 模块边界不测。已知近似是多会话附着时的注入目标；README 记录之，修法与原生 diff/jump 触发等待的是同一个 webview→host 信号。有界值（`maxTextChars`、`maxDiagnostics`、单诊断/总量上限的默认值、游标窗口）在 v1 是固定常量；待扩展有设置面后成为设置项。
