...
The handler's compiler-locked tables (`rpc-map.ts`, `UNARY_ROUTES`, `UNARY_VALUE_SCHEMAS`) forced every carrier surface to acknowledge the new method — the fixture client (`packages/client/connection/src/client/fixture.ts`) implements it as an idle no-wakeup append, so keyless web lanes exercise the frame fields. Gateway routing, provenance, and error mapping are covered by `packages/host/apiproxy/tests/api-proxy-inject.spec.ts`; the wire round-trip by `fetch-carrier.spec.ts`; schema acceptance/rejection by `rpc-schemas.spec.ts`. The no-wakeup/staging semantics of injection itself remain the agent-loop suites' property. The keyless shipped-composition smoke starts the production base and Web overlay through the Loader and HTTP carrier, pins `host.describe.protocolVersion`, proves that injection writes the durable `ide-context` source without a model request, and observes the context before the following prompt at the real LLM adapter call. The VS Code lane separately proves the editor producer and extension carrier. The `apps/vscode` `@vscode/test-electron` lane loads the built IDE client, discovers a session, and verifies that an editor opened before activation is injected durably.
